...
contract Leaderboard {
    ...
    function stakeTokens(address _user, address _token, uint256 _amount) public {
        require(checkLPTokens(_user), "entered address is not an LP token");
   
        IERC20(_token).transferFrom(msg.sender,address(this), _amount); // transfer tokens from user to contract
        ...
        if(isRegistered) {
            ...
            userBatches[_user].push(batch( _user, _token, _amount, block.timestamp, 0, 0));
            IERC20(_token).transferFrom(msg.sender, address(this), _amount);
        } else {
            // if user is not registered, register user
            // assign new user an ID number
            idToAddress[idCount] = _user;
            // push stakeData into userallStakeData
            userBatches[_user].push();
    ...
    function getIndividualScore(address _user) public view returns (uint256 score) {
        ...
        for(uint i; i < userBatch.length; i++){
